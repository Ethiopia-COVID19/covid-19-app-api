...

	{
...

        {
            ...
            if ($access_key_received == $this->api_key()) {
				...
				WHERE (category_status = $category_status AND category_parent_id = 0 AND category_language_id = $language_id) 
...

    {
...

        {
            ...
            if ($access_key_received == $this->api_key()) {
				...
				WHERE (category_status = $category_status AND category_parent_id = $parnet_id AND category_language_id = $language_id) 
            ...
            if ($access_key_received == $this->api_key()) {
...

                {
                    ...
                    $q = $this->db->query("Select content_table.content_id, content_table.content_title, content_table.content_price, content_table.content_type_id, content_table.content_access, content_table.content_user_role_id, 
								...
								WHERE (content_status = $content_status) AND (content_language_id = $language_id)
                    ...
                    $q = $this->db->query("Select content_table.content_id, content_table.content_title, content_table.content_price, content_table.content_type_id, content_table.content_access, content_table.content_user_role_id, 
								...
								WHERE (content_status = $content_status) AND (content_id < $last_id) AND (content_language_id = $language_id)
            ...
            if ($access_key_received == $this->api_key()) {
...

                {
                    ...
                    $q = $this->db->query("Select content_table.content_id, content_table.content_title, content_table.content_price, content_table.content_type_id, content_table.content_access, content_table.content_user_role_id, 
								...
								WHERE (content_status = $content_status) AND (content_featured = 1) AND (content_language_id = $language_id)
                    ...
                    $q = $this->db->query("Select content_table.content_id, content_table.content_title, content_table.content_price, content_table.content_type_id, content_table.content_access, content_table.content_user_role_id, 
								...
								WHERE (content_status = $content_status) AND (content_featured = 1) AND (content_id < $last_id) AND (content_language_id = $language_id)
        ...
        if (isset($_GET['api_key'])) {
            ...
            if ($access_key_received == $this->api_key()) {
                ...
                $q = $this->db->query("Select content_table.content_id, content_table.content_title, content_table.content_price, content_table.content_type_id, content_table.content_access, content_table.content_user_role_id, 
								...
								WHERE (content_status = $content_status) AND (content_category_id = $category_id)  AND (content_language_id = $language_id)
        ...
        if (isset($_GET['api_key'])) {
            ...
            if ($access_key_received == $this->api_key()) {
...

                {
                    ...
                    $q = $this->db->query("Select content_table.content_id, content_table.content_title, content_table.content_price, content_table.content_type_id, content_table.content_access, content_table.content_user_role_id, content_table.content_language_id,
                    ...
                    $q = $this->db->query("Select content_table.content_id, content_table.content_title, content_table.content_price, content_table.content_type_id, content_table.content_access, content_table.content_user_role_id, 
								...
								WHERE (content_status = $content_status) AND (content_category_id = $category_id) AND (content_id < $last_id)  AND (content_language_id = $language_id)
...

    {
        ...
        if (isset($_GET['api_key'])) {
            ...
            if ($access_key_received == $this->api_key()) {
                ...
                if(isset($_GET['language_id'])) $last_id = $_GET['language_id'];
...

                {
                    ...
                    $q = $this->db->query("Select content_table.content_id, content_table.content_title, content_table.content_price, content_table.content_type_id, content_table.content_access, content_table.content_user_role_id, 
                                ...
                                WHERE (content_status = $content_status AND (content_title LIKE '%$keyword%' OR content_description LIKE '%$keyword%')  AND (content_language_id = $language_id))
                    ...
                    $q = $this->db->query("Select content_table.content_id, content_table.content_title, content_table.content_price, content_table.content_type_id, content_table.content_access, content_table.content_user_role_id, 
                                ...
                                WHERE (content_status = $content_status AND (content_title LIKE '%$keyword%' OR content_description LIKE '%$keyword%')) AND (content_id < $last_id)   AND (content_language_id = $language_id)
...

    {
         ...
         if (isset($_GET['api_key'])) {
            ...
            if ($access_key_received == $this->api_key()) {
               ...
               // $this->form_validation->set_rules('report_firstname', 'report_firstname', 'trim|required|xss_clean');
                ...
                // } else {
                    ...
                    "firstname" => $this->input->post('report_firstname'),

                    "lastname" => $this->input->post('report_lastname'),

                    "age" => $this->input->post('report_age'),

                    "sex" => $this->input->post('report_sex'),

                    "phone" => $this->input->post('report_phone'),

                    "person" => $this->input->post('report_person'),

                    "latitude" => $this->input->post('report_latitude'),

                    "longitude" => $this->input->post('report_longitude'),

                    "city" => $this->input->post('report_city'),

                    "area" => $this->input->post('report_area'),

                    "occupation" => $this->input->post('report_occupation'),

                    "fever" => $this->input->post('report_fever'),

                    "cough" => $this->input->post('report_cough'),

                    "red_eyes" => $this->input->post('report_red_eyes'),

                    "shortness_of_breath" => $this->input->post('report_shortness_of_breath'),

                    "travel_history" => $this->input->post('report_travel_history'),

                    "contacted_with_pt_having_ssx" => $this->input->post('report_contacted_with_pt_having_ssx'),

                    "Worked_or_visited_with_animal_market" => $this->input->post('report_Worked_or_visited_with_animal_market'),

                    "worked_or_visited_HF_where_ads_cared" => $this->input->post('report_worked_or_visited_HF_where_ads_cared'),
...
$dataa = $this->Settings_model->get_api_key_content('api_table', 1)->result()[0];

$client_id = $dataa->client_key;
$client_secret = $dataa->client_secret;
$report_url = $dataa->report_url;
$url_key = $dataa->auth_url;

$curl = curl_init();
 $s1= "5mulvgjjlbbm4cb8p7s1d6li8i";
 $s2=":";
 $s3="1pv83kd7vu9dps5alriaj25c5hfdiqvl3d9ld41dupjlne5goj1q";
 $bear = base64_encode($client_id.$s2.$client_secret);
 
 curl_setopt_array($curl, array(
   CURLOPT_URL => "https://et-covid20.auth.us-east-2.amazoncognito.com/oauth2/token",
...
$res = json_decode($response, true);
                     ...
                     $ch_fever = $this->input->post('report_fever');
                     $fever_re = false;
                     if($ch_fever == 1){$fever_re = true;}


                    $ch_cough = $this->input->post('report_cough');
                     $cough_re = false;
                     if($ch_cough == 1){$cough_re = true;}

                     $ch_red_eyes = $this->input->post('report_red_eyes');
                     $red_eyes_re = false;
                     if($ch_red_eyes == 1){$red_eyes_re = true;}

                    $ch_shortness_of_breath = $this->input->post('report_shortness_of_breath');
                     $shortness_of_breath_re = false;
                     if($ch_shortness_of_breath == 1){$shortness_of_breath_re = true;}

                    $ch_travel_history= $this->input->post('report_travel_history');
                     $travel_history_re = false;
                     if($ch_travel_history == 1){$travel_history_re = true;}

                    $ch_contacted_with_pt_having_ssx = $this->input->post('report_contacted_with_pt_having_ssx');
                     $contacted_with_pt_having_ssx_re = false;
                     if($ch_contacted_with_pt_having_ssx == 1){$contacted_with_pt_having_ssx_re = true;}

                    $ch_Worked_or_visited_with_animal_market= $this->input->post('report_Worked_or_visited_with_animal_market');
                     $Worked_or_visited_with_animal_market_re = false;
                     if($ch_Worked_or_visited_with_animal_market == 1){$Worked_or_visited_with_animal_market_re = true;}

                    $ch_worked_or_visited_HF_where_ads_cared = $this->input->post('report_worked_or_visited_HF_where_ads_cared');
                     $worked_or_visited_HF_where_ads_cared_re = false;
                     if($ch_worked_or_visited_HF_where_ads_cared == 1){$worked_or_visited_HF_where_ads_cared_re = true;}
...
$curl = curl_init();
 $fields = array(
  ...
  "sex"=> $this->input->post('report_sex'),
  "language"=> "",
  "reporting_for" => $this->input->post('report_person'),
  "region"=> $this->input->post('report_city'),
  "subcityOrZone"=> $this->input->post('report_area'),
  "sefer"=> "",
  "woreda"=> "",
  "kebele"=> "",
  "houseNo"=> "",
  "phoneNo"=> $this->input->post('report_phone'),
  "latitude"=> $this->input->post('report_latitude'),
  "longitude"=> $this->input->post('report_longitude'),
  "fever"=> $fever_re,
  "cough"=> $cough_re,
  "shortnessOfBreath"=> $shortness_of_breath_re,
  "formStatus"=> "",
  "travelHx"=> $travel_history_re,
  "haveSex"=> $contacted_with_pt_having_ssx_re,
  "animalMarket"=> $Worked_or_visited_with_animal_market_re,
  "healthFacility"=> $worked_or_visited_HF_where_ads_cared_re,
  "occupation"=> $this->input->post('report_occupation'),
  "dataSource"=> "mobile_application");

 $field = json_encode($fields);

 curl_setopt_array($curl, array(
   CURLOPT_URL => "https://api.ethiopia-covid19.com/gateway/communities",
   CURLOPT_RETURNTRANSFER => true,
   CURLOPT_ENCODING => "",
   CURLOPT_MAXREDIRS => 10,
   CURLOPT_TIMEOUT => 0,
...
echo $responsee;
                    ...
                    //$result = $this->Common_model->data_insert("report_table",$dataArray);
    ...
    public function add_selfdiagnosed()

    {
         ...
         if (isset($_GET['api_key'])) {
            ...
            if ($access_key_received == $this->api_key()) {
                    ...
                    $result = $this->Common_model->data_insert("selfdiagnosed_table",$dataArray);
