...

{
...

    {
        ...
        $data["reportsCount"] = $this->Report_model->all_reports_count();

        $data["maleCount"] = $this->Dashboard_model->all_male_count();

        $data["femaleCount"] = $this->Dashboard_model->all_female_count();
...

    {
        ...
        $this->load->view('dashboard/Report/users_list_view', $data);
...

    {
		...
		$data["usersList2"] = $this->Report_model->get_users_list2('report_table')->result();

        $data["pageTitle"] = $this->lang->line("Report List");

        $this->load->view('dashboard/Report/users_list2_view', $data);

    }

     public function users_list3()

    {
        ...
        $data["usersList2"] = $this->Report_model->get_users_list3('push_notifications')->result();

        $data["pageTitle"] = $this->lang->line("Push Notifications List");

        $this->load->view('dashboard/Report/users_list3_view', $data);
...

    {
        ...
        $totalData = $this->Report_model->all_report_count();
...

        {
            ...
            $users = $this->Report_model->all_reports($limit,$start,$order,$dir);
   ...
   public function add_report()
         ...
         if (isset($_GET['api_key'])) {
            ...
            if ($access_key_received == $this->api_key()) {
               ...
               //  } else {
                    ...
                    "symptoms" => $this->input->post('fever'),

                    "symptoms" => $this->input->post('cough'),

                    "symptoms" => $this->input->post('shortness_of_breath'),

                    "travel_history" => $this->input->post('travel_history'),

                    "contacted_with_pt" => $this->input->post('contacted_with_pt_having_ssx'),

                    "contacted_with_pt" => $this->input->post('Worked_or_visited_with_animal_market'),

                    "worked_or visited_HF" => $this->input->post('worked_or visited_HF_where_ads_cared'),
...

    {
        ...
        $data["maleCount"] = $this->Report_model->all_male_count();

        $data["femaleCount"] = $this->Report_model->all_female_count();

        $data["selfdiagnosedcount"] = $this->Report_model->selfdiagnosedcount();

        $data["usersActivity"] = $this->User_model->get_all_activity('activity_table', 500)->result();

        $data["pageTitle"] = $this->lang->line("Activity");

        $this->load->view('dashboard/Report/users_activity_view', $data);
